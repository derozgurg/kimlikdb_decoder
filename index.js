...
function decyrptRow(row){
   	var nr = {};
   	for(var ho in row) nr[ho] = decodeString(row[ho]);
   	return nr;
}

connection.connect();

connection.query('SELECT * FROM rnd LIMIT 1', function(err, rows, fields) {
  	if (err) throw err;
  	console.log('The solution is: ', decyrptRow(rows[0]));
});
connection.end();
